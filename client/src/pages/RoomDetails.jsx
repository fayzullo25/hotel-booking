    ...
    <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="flex flex-col lg:flex-row mt-6 gap-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:w-1/2 w-full">
          {room?.images.length >1 && room.images.map((roomImage,index)=>(
            <img onClick={()=>setMainImage(roomImage)} key={index} src={roomImage} alt="Room Image" className={`w-full rounded-xl shadow-md object-contain cursor-pointer ${mainImage===roomImage} && outline-3 outline-orange-500`} />
          ))}
        </div>
      </div>
      {/* room highlights */}
      ...
      <form className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl">
              <div className="flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500">
                ...
                <div className="flex flex-col">
                  ...
                  <input type="number" id="guests" placeholder="0" className="max-w-20 rounded border border-gray-300 px-3 py-4 mt-1.5 outline-none " required/>
                </div>
              </div>
